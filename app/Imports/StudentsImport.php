    ...
    public function model(array $row)
    {
        // dd(Date::excelToDateTimeObject($row["tanggal_lahir"])->format("Y-m-d"));
            return new Student([
                'nis' => intval($row["nis"]),
                'name' => $row["nama"],
                'born_city' => $row["tempat_lahir"],
                'address' => $row["alamat"],
                'born_date' => Date::excelToDateTimeObject(intval($row["tanggal_lahir"]))->format("Y-d-m"),
                'gender' => $row["jk"],
                'blood_type' => $row["gol_darah"],
                'school_from' => $row["asal_sekolah"],
                'classroom_id' => intval($row["id_kelas"]),
                'father_name' => $row["nama_ayah"],
                'mother_name' => $row["nama_ibu"],
                'guardian_name' => $row["nama_wali"],
                'no_bpjs' => intval($row["no_bpjs"] ),
                'faskes_bpjs' => intval($row["faskes_bpjs"]),
